...
const AvaliacaoPessoaSchema = new mongoose.Schema({
  ...
  proatividade:       { type: Number, required: true, min: 0, max: 10 },
  lideranca:          { type: Number, required: true, min: 0, max: 10 },
  escutaAtiva:        { type: Number, required: true, min: 0, max: 10 },
  iniciativa:         { type: Number, required: true, min: 0, max: 10 },
  trabalhoEquipe:     { type: Number, required: true, min: 0, max: 10 },
  comunicacao:        { type: Number, required: true, min: 0, max: 10 },
  resolucaoProblemas: { type: Number, required: true, min: 0, max: 10 },
  adaptabilidade:     { type: Number, required: true, min: 0, max: 10 },
  engajamento:        { type: Number, required: true, min: 0, max: 10 },
